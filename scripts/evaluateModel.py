...
def getCellDicts(imgDir, baseDir):
    ...
    for idx, img in enumerate(imgs):
        ...
        phaseContrastName = os.path.join(
            baseDir, "PhaseContrast", "PhaseContrast_" + imgBase + ".jpg"
        )
        ...
        imgMaskName = os.path.join(
            baseDir, "MasksLabeled", "MasksLabeled_" + imgBase + ".tif"
        )
        ...
        for maskLabel, fluorescence in zip(annos["maskLabel"], annos["fluorescence"]):
            ...
            contours = measure.find_contours(img_as_float(imgMask == maskLabel), 0.5)
            hull = []
...
DatasetCatalog.register(
    "cellMorph_" + "train", lambda x=inputs: getCellDicts(inputs[0], inputs[1])
)
...
DatasetCatalog.register(
    "cellMorph_" + "Validate", lambda x=inputs: getCellDicts(inputs[0], inputs[1])
)
