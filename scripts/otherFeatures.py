...
cells = cellMorphHelper.filterCells(
    cells, confluencyDate=datetime.datetime(2022, 4, 8, 16, 0), color="green", edge=True
)
# %% Pyfeatures test
...
for cell in cellSample[0:20]:
    ...
    try:
        extractedFeatures, extractedLabels = cellMorphHelper.extractFeatures(
            image, mask, perimeter
        )
    ...
    if cellNum % 100 == 0:
        ...
        pickle.dump(
            [allFeatures, allLabels], open("../results/allFeaturesTJ2201.pickle", "wb")
        )
    cellNum += 1
# %%
cell = cellSample[1]
