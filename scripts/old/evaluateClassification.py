...
def getCellDicts(expDir, stage):
    ...
    for img in imgs:
        ...
        for splitNum in range(1, 5):
            phaseContrastName = (
                "phaseContrast_" + imgBase + "_" + str(splitNum) + ".jpg"
            )
            ...
            for maskLabel, fluorescence in zip(
                annos["maskLabel"], annos["fluorescence"]
            ):
                # Contour converts the mask to a polygon
                contours = measure.find_contours(
                    img_as_float(imgMask == maskLabel), 0.5
                )
...
DatasetCatalog.register(
    "cellMorph_" + "val", lambda x=inputs: getCellDicts(inputs[0], inputs[1])
)
